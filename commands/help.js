    ...
    async execute(interaction) {
        ...
        if (commandName) {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(command.data.description || 'No description provided.'); // Command description or a default message
            ...
            if (command.data.options && command.data.options.length > 0) {
                ...
                embed.addFields({ name: 'Options', value: options }); // Add a field for options
            }
            ...
            const commands = client.commands.map(command => `\`/${command.data.name}\` - ${command.data.description || 'No description.'}`);

            // Create an embed message to list all available commands
            const embed = new EmbedBuilder()
                .setColor('#0099ff') // Blue color
                .setTitle('🤖 Available Commands') // Title of the embed
                .setDescription(
